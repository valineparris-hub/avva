...
const SUBJECTS: SubjectInfo[] = [
  {
    ...
    description: 'Numbers, operations, fractions, geometry & problem solving',
  },
  {
    name: 'Language Arts',
    icon: BookOpen,
    ...
    description: 'Grammar, comprehension, reading & writing skills',
  },
  {
    name: 'Science',
    icon: FlaskConical,
    ...
    description: 'Living things, matter, energy & the natural world',
  },
  {
    name: 'Social Studies',
    icon: Globe2,
    ...
    description: 'History, geography, government & Caribbean culture',
  },
];

const PASS_THRESHOLD = 85;
const PRIMARY_SECONDARY_LEVELS = new Set([
  'primary_infants_1', 'primary_infants_2',
  'standard_1', 'standard_2', 'standard_3', 'standard_4', 'standard_5',
  'form_1', 'form_2', 'form_3',
]);

export function SubjectDashboard({ profile, onBack }: SubjectDashboardProps) {
  ...
  useEffect(() => {
    // Auto-expand the first subject that has an unlocked, uncompleted module
    if (!expandedSubject && !loading) {
      const firstSubject = SUBJECTS.find((s) => {
      ...
      setExpandedSubject(firstSubject?.name ?? SUBJECTS[0].name);
    }
  ...
  function getSubjectProgress(subjectName: string) {
    ...
    return { completed, total, pct, avgScore, nextUnlocked, mods };
  }
  ...
  async function handleActivityComplete(mod: ModuleWithProgress, score: number, passed?: boolean) {
    ...
    // For quizzes, only mark as 'completed' if passed (>=85%)
    // If not passed, record as 'in_progress' with the score
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-50 to-ocean-50/30">
          ...
          <div className="bg-gradient-to-br from-brand-600 via-brand-700 to-ocean-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-brand-600/20 animate-slide-up overflow-hidden relative">
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-5">
                <div>
                  <p className="text-white/70 text-sm font-medium mb-1">{profile.name}'s Learning Journey</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {SUBJECTS.map((subject) => {
              ...
              return (
                <div
                  key={subject.name}
                  className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden animate-slide-up"
                >
                  {/* Subject header — clickable to expand/collapse */}
                  <button
                    onClick={() => setExpandedSubject(isExpanded ? null : subject.name)}
                    className={`w-full bg-gradient-to-br ${subject.gradient} p-5 text-white relative overflow-hidden text-left`}
                  >
                    ...
                    <div className="relative flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-white/70">{data.completed}/{data.total} modules · {data.pct}%</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <span className="text-2xl font-bold font-display tabular-nums">{data.pct}%</span>
                        {data.avgScore > 0 && (
                          <p className="text-xs text-white/60">{data.avgScore}% avg</p>
                        )}
                      </div>
                    </div>
                  </button>
                  ...
                  {isExpanded && (
                    <div className="p-5 animate-fade-in">
                            ...
                            >
                              ...
                              >
                                {locked ? (
                                  ...
                                  <PlayCircle className="w-5 h-5" style={{ color: failedQuiz ? '#d97706' : subject.color }} />
                                ) : (
                                  <span>{idx + 1}</span>
                                )}
                              </div>
                      ...
                      {/* Next up / complete footer */}
                      {data.nextUnlocked ? (
                        <button
                          onClick={() => setActiveModule(data.nextUnlocked!)}
                          className="mt-3 w-full flex items-center justify-between px-3 py-2.5 bg-gradient-to-r from-slate-50 to-slate-100 hover:from-brand-50 hover:to-ocean-50 rounded-xl transition-all group"
                        >
                    ...
                    <div className="px-5 py-3">
                      ...
                      {data.nextUnlocked ? (
                        ...
                        <p className="text-xs text-accent-600 font-medium mt-1.5 flex items-center gap-1">
                          <Award className="w-3 h-3" /> Subject mastered!
                        </p>
                      ) : null}
                    </div>
                  )}
